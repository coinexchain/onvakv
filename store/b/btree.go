...
func DeletedValue() Value {
	return Value{obj: &deletedMarker}
}
...
func (v Value) GetObj() interface{} {
	if v.obj == &deletedMarker {
		return nil
	}
	return v.obj
}
...
func (t *Tree) First() (k []byte, v Value) {
	if q := t.first; q != nil {
		q := &q.d[0]
		k, v = q.k, q.v
	}
...
func (t *Tree) Get(k []byte) (v Value, ok bool) {
	q := t.r
	if q == nil {
		return
	}
...
func (t *Tree) insert(q *d, i int, k []byte, v Value) *d {
	t.ver++
	c := q.c
	if i < c {
...
// (zero-value, zero-value) if the tree is empty.
func (t *Tree) Last() (k []byte, v Value) {
	if q := t.last; q != nil {
		q := &q.d[q.c-1]
		k, v = q.k, q.v
	}
...
func (t *Tree) overflow(p *x, q *d, pi, i int, k []byte, v Value) {
	t.ver++
	l, r := p.siblings(pi)
...
func (t *Tree) Set(k []byte, v Value) {
	//dbg("--- PRE Set(%v, %v)\n%s", k, v, t.dump())
	//defer func() {
	//	dbg("--- POST\n%s\n====\n", t.dump())
	//}()
...
func (t *Tree) Put(k []byte, upd func(oldV Value, exists bool) (newV Value, write bool)) (oldV Value, written bool) {
	pi := -1
	var p *x
	q := t.r
	var newV Value
...
func (t *Tree) split(p *x, q *d, pi, i int, k []byte, v Value) {
	t.ver++
	r := btDPool.Get().(*d)
	if q.n != nil {
		r.n = q.n
...
// io.EOF is returned.
func (e *Enumerator) Next() (k []byte, v Value, err error) {
...
// == io.EOF is returned.
func (e *Enumerator) Prev() (k []byte, v Value, err error) {
	if err = e.err; err != nil {
		return
	}
